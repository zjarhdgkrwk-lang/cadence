...
- **창 X = 앱 종료. 트레이 상주는 v1 비범위.** (트레이 숨김은 작업 표시줄 미표시·강제 종료 필요 등 사용성 문제로 채택 안 함)
- 새 기능/의존성을 임의로 추가하지 않는다.
- 모든 기능은 파일 기반 통합 로그(app_log_dir/logs/cadence.log)에 기록한다.
  Rust(tracing)와 프런트(console/onerror를 IPC로 전달, [FE] 태그)를 한 파일에 합친다.
- 새 기능을 추가할 때마다 핵심 분기·실패 지점에 로그를 남긴다(예: 로드 src,
  audio.error code, IPC 실패, asset/scope 거부, 패닉).
- 사용자가 버그를 보고할 때 이 로그 파일 하나로 진단 가능해야 한다.
- 로그는 일자별 롤링 + 최근 며칠만 보관. 콘솔에도 동시 출력.
